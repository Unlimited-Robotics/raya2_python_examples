...
class RayaApplication(RayaApplicationBase):
    async def setup(self):
        ...
        for object in self.callback_object:
            if object not in self.detector.get_objects_names():
                self.log.info((f'\'{self.callback_object}\' is not a valid'
        ...
        await self.detector.find_objects(self.callback_object, self.callback_obj)
        self.time_counter = 0
    ...
    def callback_obj(self, obj):
        self.log.info(f"{obj['object_name']} detected")
        # Cancel find Objects
    ...
    def get_args(self):
        ...
        parser.add_argument('-c',  '--camera-name', type=str, help='name of camera to use')
        parser.add_argument("-co", "--callback-object",default=['apple', 'book'],nargs='+', help="list of strings with the objects to detect in not blockin mode")
        ...
        self.callback_object = args.callback_object
