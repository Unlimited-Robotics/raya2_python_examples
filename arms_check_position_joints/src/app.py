    ...
    async def setup(self):
        ...
        if self.list_arms:
            ...
            if self.joint_values is not None:
                ...
                print(
                    f"\n{self.col.FAIL}DEFINE THE JOINTS VALUES ARRAY TO VALIDATE(-j){self.col.ENDC}"
                )
...
option is true the program just print the list of avaliable arms",
    ...
    def callback_finish_srv(self, error, error_msg, distance):
        if distance != "" and error == 0:
            print(
                f"\n{self.col.OKGREEN}FINISH VALIDATION POSITION JOINTS ARE VALID AND THE DISTANCE \
FROM THE ACTUAL POSITION IS: {distance:.2f}{self.col.ENDC}"
            )
        else:
            print(
                f"\n{self.col.FAIL}FINISH VALIDATION POSITION JOINTS ARE INVALID{self.col.ENDC}"
            )
        print("------------")
        print("")
